            ...
            <div className="shadow-lg rounded-lg overflow-hidden bg-gray-900 mt-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-6">
                    {[
                        ...
                        { label: "As Exit - Can Route IP External v4", value: data.last_probe_result.outcome.as_exit.can_route_ip_external_v4 ? '🟢 Yes' : '🔴 No' },
                        { label: "As Exit - Can Route IP External v6", value: data.last_probe_result.outcome.as_exit.can_route_ip_external_v6 ? '🟢 Yes' : '🔴 No' },
                        { label: "As Exit - Can Route IP v4", value: data.last_probe_result.outcome.as_exit.can_route_ip_v4 ? '🟢 Yes' : '🔴 No' },
                        { label: "As Exit - Can Route IP v6", value: data.last_probe_result.outcome.as_exit.can_route_ip_v6 ? '🟢 Yes' : '🔴 No' },
                        { label: "WG - Can Handshake", value: data.last_probe_result.outcome.wg.can_handshake ? '🟢 Yes' : '🔴 No' },
                        { label: "WG - Can Register", value: data.last_probe_result.outcome.wg.can_register ? '🟢 Yes' : '🔴 No' },
                        { label: "WG - Can Resolve DNS", value: data.last_probe_result.outcome.wg.can_resolve_dns ? '🟢 Yes' : '🔴 No' },
                        { label: "WG - Ping Hosts Performance", value: data.last_probe_result.outcome.wg.ping_hosts_performance },
                        { label: "WG - Ping IPs Performance", value: data.last_probe_result.outcome.wg.ping_ips_performance },
                    ].map((item, index) => (
                        <div key={index} className="p-4 bg-gray-800 rounded-lg shadow-md">
                            <h3 className="text-xl font-semibold">{item.label}:</h3>
                            <p className={typeof item.value === 'string' && item.value.includes('🟢') ? 'text-green-500' : typeof item.value === 'string' && item.value.includes('🔴') ? 'text-red-500' : ''}>{item.value}</p>
                        </div>
                    ))}
